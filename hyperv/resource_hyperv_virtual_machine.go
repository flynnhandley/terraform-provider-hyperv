...
func resourceHypervVMCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if vm.VlanID != 0 {
		if err = hvDriver.SetVirtualMachineVlanID(vm.Name, vm.VlanID); err != nil {
			return err
		}
	}
	...
	// Sets Secure Boot function
	if vm.Generation == 2 {
		if err = hvDriver.SetVirtualMachineSecureBoot(vm.Name, vm.EnableSecureBoot, vm.SecureBootTemplate); err != nil {
			return err
		}
	}

	// Sets Dynamic Memory function
	if err = hvDriver.SetVirtualMachineDynamicMemory(vm.Name, vm.EnableDynamicMemory); err != nil {
		return err
	}
	...
	if vm.NetworkAdapters != nil {
		for _, a := range vm.NetworkAdapters {
			...
			if a.MAC != "" {
				if err = hvDriver.SetNetworkAdapterStaticMacAddress(vm.Name, a.Name, a.MAC); err != nil {
					return err
				}
			}
		}
